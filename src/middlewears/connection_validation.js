...
const status=['pending','ignored','accepted']
...
try {
    const result=  await Connection.findOne({ $or : 
        [
        {sender_id : sender_id, receiver_id :  req.user._id }, 
        {sender_id :  req.user._id, receiver_id : sender_id}
     ]})
     console.log(result);
        req.status=  result? result.status : (req.params.status=='interested' ? 'pending' : req.params.status) ;
        console.log(req.status);
    if(! result || status.includes(result.status)) {

        if (result && req.method === 'POST' && result.status === 'pending') {
            return res.json({
